package project.libraryclient.JDBC;

import project.libraryclient.Consts.JDBCData;
...
public class AppJDBC {
    ...
    public ResultSet QueryLoginUserMail(String InputMail, String PassWord) throws SQLException {
        // only for user account
        String query = "select count(*) from user u join password p on u.id = p.id where u.mail = ? and p.password = ? and u.accounttype = 'user';";
